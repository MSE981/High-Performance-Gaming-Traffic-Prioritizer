...
// §2.3.7  Hardware checks via raw fd (/sys, /proc) — no ifstream
#include <array>
#include <cstring>
#include <functional>
#include <thread>
...
private:
    // ── worker ──────────────────────────────────────────────────────────────
    // §3.3.1: This is the thread worker. Runs all sub-tests in sequence,
    // then fires the callback (§3.3.2: signals completion to caller).
    void run() {
        Report r;
        test_nat(r);
        test_dns(r);
    ...
    // Returns buffer and actual length via out_len
    static std::array<uint8_t, 512> make_dns_query(uint32_t sip, uint32_t dip,
        ...
        // QNAME wire format: encode "label.label\0"
    ...
    // DHCP DISCOVER: 42-byte UDP (src=68, dst=67) + 236-byte DhcpHeader + options
    static std::array<uint8_t, 512> make_dhcp_discover(size_t& out_len) {
        ...
        ipv4->ver_ihl  = 0x45;
        ipv4->protocol = 17;
        ipv4->saddr    = 0;           // 0.0.0.0 (DISCOVER)
        ipv4->daddr    = 0xFFFFFFFF;  // broadcast

        udp->source = htons(68);
        udp->dest   = htons(67);

        // DhcpHeader at offset 42
        auto* dhcp = reinterpret_cast<Logic::DhcpHeader*>(buf.data() + 42);
        ...
        // DHCP options at offset 42 + 236 = 278
        size_t opt = 42 + sizeof(Logic::DhcpHeader);
    ...
    void test_nat(Report& r) {
        ...
        uint32_t wan_ip = Config::parse_ip_str("10.0.0.1");
        nat->set_wan_ip(wan_ip);

        uint32_t lan_ip = Config::parse_ip_str("192.168.1.100");
        uint32_t ext_ip = Config::parse_ip_str("8.8.8.8");
    ...
    void test_dns(Report& r) {
        ...
        size_t pkt_len = 0;
        uint32_t cli = Config::parse_ip_str("192.168.1.100");
        uint32_t srv = Config::parse_ip_str("8.8.8.8");
        ...
        uint32_t upstream = Config::parse_ip_str("9.9.9.9");
        dns_redir->set_upstream(upstream, 0);
    ...
    void test_dhcp(Report& r) {
        Logic::DhcpEngine dhcp("192.168.1.1");
    ...
    void test_firewall(Report& r) {
        ...
        // saddr=srv_ip, daddr=lan_ip, sport=80, dport=54321
