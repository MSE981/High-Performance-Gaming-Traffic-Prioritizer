...
bool ForwardingState_util::is_invalid_nat_wan(Utils::Net::IPv4Net a) noexcept {
    if (a.raw() == 0) return true;
    const uint32_t h = ntohl(a.raw());
    return (h >> 24) == 127;
}

std::expected<Utils::Net::IPv4Net, std::string> ForwardingState_util::resolve_nat_wan_ip() const {
    std::string s = Utils::Network::Network_util::get_local_ip(Config::iface_wan());
    if (s.empty())
...
void ForwardingState_util::refresh_l2() {
    ...
    const bool lan_ok = Utils::Network::Network_util::get_iface_hwaddr(Config::iface_lan(), snap.lan_hw.data());
    const bool wan_ok = Utils::Network::Network_util::get_iface_hwaddr(Config::iface_wan(), snap.wan_hw.data());

    snap.wan_prefix_len = static_cast<int32_t>(
        Utils::Network::Network_util::get_iface_ipv4_prefix_len(Config::iface_wan()));
    if (auto wan = resolve_nat_wan_ip()) {
        snap.wan_ip_nbo = wan->raw();
    } else {
        snap.wan_ip_nbo = 0;
    }
    snap.wan_cfg_valid = snap.wan_ip_nbo != 0 && !is_invalid_nat_wan(Utils::Net::IPv4Net{snap.wan_ip_nbo})
        && snap.wan_prefix_len >= 0 && snap.wan_prefix_len <= 32;

    std::string gw_ip = Utils::Network::Network_util::get_default_gateway_for_iface(Config::iface_wan());
    if (gw_ip.empty())
        gw_ip = Utils::Network::Network_util::get_gateway_ip();
    snap.default_gw_ip_configured = !gw_ip.empty();
    if (!gw_ip.empty())
        Utils::Network::Network_util::force_arp_resolution(gw_ip);

    Utils::Network::ArpTableRow rows[ForwardingState_util::L2Snapshot::MAX_ARP];
    const size_t n = Utils::Network::Network_util::read_arp_table(rows, ForwardingState_util::L2Snapshot::MAX_ARP);
